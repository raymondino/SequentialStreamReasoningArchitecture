/*
 * @Author: Rui Yan
 * to-dos:
 *       1. calculate the precision, recall & f-measure for each run. 
 */
package com.pnnl.shyre.main;

import java.io.BufferedReader;
import java.io.FileReader;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.HashSet;
import java.util.LinkedHashMap;
import java.util.Map;

import com.pnnl.shyre.stardog.cache.Cache;
//import com.pnnl.shyre.stardog.cache.StardogFEFO;
//import com.pnnl.shyre.stardog.cache.StardogLFU;
//import com.pnnl.shyre.stardog.cache.StardogLRU;

public class Main {
	
	private static String serverURL = "snarl://localhost:5820";
	private static String username = "admin";
	private static String password = "admin";
		
	public static void main(String[] args) throws Exception {
	
		// load ground truth
		HashSet<String> groundTruth = new HashSet<String>();
		BufferedReader br = new BufferedReader(
				            new FileReader("files/LUBM45groundTruth.txt"));
		String line;
		while((line = br.readLine()) != null) {
			groundTruth.add(line);
		}
		br.close();
		System.out.println("[INFO] ground truth loaded");
		
	
		
		
		// Map<cacheSize, evictionAmount>
		Map<Integer, ArrayList<Integer>> cases = 
				new LinkedHashMap<Integer, ArrayList<Integer>> (4); 

		cases.put(1000, new ArrayList<Integer>
    	(Arrays.asList(250, 500, 750, 1000)));
		cases.put(10, new ArrayList<Integer>
		(Arrays.asList(2, 5, 7, 10)));  
		cases.put(100, new ArrayList<Integer>
		(Arrays.asList(25, 50, 75, 100)));

		int evictTimes = 20;
		String backgroundOntologyPath = "files/univ-bench.owl";
		String disk = "disk";
		String memory = "mem";

		// number of triples in a graph: 1, 10, 100, 1000
		for(int numberOfTriples_ = 1; // 1
				numberOfTriples_ < 1001; // 1001
				numberOfTriples_*=10) {
			
			Cache sdfefomem = new Cache(numberOfTriples_, 
					evictTimes, groundTruth, "FEFO",  memory, 
					Main.serverURL, Main.username, Main.password);
			Cache sdlfumem = new Cache(numberOfTriples_, 
					evictTimes, groundTruth, "LFU", memory,
					Main.serverURL, Main.username, Main.password);
			Cache sdlrumem = new Cache(numberOfTriples_, 
					evictTimes, groundTruth, "LRU", memory,
					Main.serverURL, Main.username, Main.password);

			Cache sdfefodisk = new Cache(numberOfTriples_, 
					evictTimes, groundTruth, "FEFO", disk,
					Main.serverURL, Main.username, Main.password);
			Cache sdlfudisk = new Cache(numberOfTriples_, 
			evictTimes, groundTruth, "LFU", disk,
					Main.serverURL, Main.username, Main.password);
			Cache sdlrudisk = new Cache(numberOfTriples_, 
			evictTimes, groundTruth, "LRU", disk,
					Main.serverURL, Main.username, Main.password);	
			
/*			StardogFEFO sdfefomem = new StardogFEFO(numberOfTriples_, 
					evictTimes, groundTruth, memory, 
					Main.serverURL, Main.username, Main.password);
			StardogLFU sdlfumem = new StardogLFU(numberOfTriples_, 
					evictTimes, groundTruth, memory,
					Main.serverURL, Main.username, Main.password);
			StardogLRU sdlrumem = new StardogLRU(numberOfTriples_, 
					evictTimes, groundTruth, memory,
					Main.serverURL, Main.username, Main.password);

			StardogFEFO sdfefodisk = new StardogFEFO(numberOfTriples_, 
					evictTimes, groundTruth, disk,
					Main.serverURL, Main.username, Main.password);
			StardogLFU sdlfudisk = new StardogLFU(numberOfTriples_, 
			evictTimes, groundTruth, disk,
					Main.serverURL, Main.username, Main.password);
			StardogLRU sdlrudisk = new StardogLRU(numberOfTriples_, 
			evictTimes, groundTruth, disk,
					Main.serverURL, Main.username, Main.password);			
*/			
			for(Map.Entry<Integer, ArrayList<Integer>> entry :
				cases.entrySet()) {
				sdfefomem.setSize(entry.getKey());
				sdlfumem.setSize(entry.getKey());
				sdlrumem.setSize(entry.getKey());
				
				sdfefodisk.setSize(entry.getKey());
				sdlfudisk.setSize(entry.getKey());
				sdlrudisk.setSize(entry.getKey());

				ArrayList<Integer> evictArray = entry.getValue();
				for(int i : evictArray){
					sdfefomem.setEvictAmount(i);
					sdlfumem.setEvictAmount(i);
					sdlrumem.setEvictAmount(i);
					
					sdfefomem.run(backgroundOntologyPath);
					sdlrumem.run(backgroundOntologyPath);
					sdlfumem.run(backgroundOntologyPath);
					
					sdfefodisk.setEvictAmount(i);
					sdlfudisk.setEvictAmount(i);
					sdlrudisk.setEvictAmount(i);
					
					sdfefodisk.run(backgroundOntologyPath);
					sdlrudisk.run(backgroundOntologyPath);
					sdlfudisk.run(backgroundOntologyPath);
				}
			}
			sdfefomem.clean();
			sdlfumem.clean();
			sdlrumem.clean();
			
			sdfefodisk.clean();
			sdlfudisk.clean();
			sdlrudisk.clean();
		}
	}
}
