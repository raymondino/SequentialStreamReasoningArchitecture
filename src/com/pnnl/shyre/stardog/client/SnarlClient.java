package com.pnnl.shyre.stardog.client;
...
public class SnarlClient {
	...
	// constructor
	public SnarlClient(String serverURL_, String dbName_, String username_, String password_) {
		...
		System.out.println("[INFO] connection to " + serverURL + "/" + dbName + " is established");
	}
	...
	public void countTriples() throws QueryEvaluationException {
		TupleQueryResult aResult = aConn.select("select (count (distinct *) as ?count) where { graph ?g {?s ?p ?o.} }").execute();
		String count = aResult.next().getBinding("count").toString();
		aResult.close();
		System.out.println("[info] " + count + " triples in the database " + dbName);
	}
